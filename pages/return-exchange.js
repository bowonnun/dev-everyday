...
class termsConditions extends Component{
    ...
    render(){
        ...
        const {posts} = this.state;
        return(
            <Page title="Return & Exchange" description="จุดมุ่งหมายของแบรนด์ Karmakamet ในปีนี้เรามุ้งเน้นที่จะพูดเรื่อง “Life Remedy” คือการใช้ชีวิตอันแสนรื่นรมณ์ในแบบฉบับของ Karmakamet Aromati"  images="/static/images/icon-logo/LOGO_circle.png" keywords="เครื่องหอม น้ำมันหอม จุดมุ่งหมายของแบรนด์ Karmakamet ">
                {
                    posts != null ?
...
export async function getServerSideProps({params,req}) {
    ...
    let posts = await getPage('en', ' return-exchange')
    let poststh = await getPage('th', ' return-exchange')
    if(isServer){
